...
(function(){
  ...
  const apply=async()=>{
    try{
      ...
      const s=r.data[0];
      const color=(s.warna||fallback).trim();
      document.documentElement.style.setProperty('--brand-color', color);
      document.documentElement.style.setProperty('--green', color);
      const name=(s.nama_usaha||'Familia Food').trim();
      document.querySelectorAll('.brand,.logo').forEach(el=>{
        const logo=(s.logo||'').trim();
        el.innerHTML=logo
          ? `<img src="${logo.replace(/&/g,'&amp;').replace(/"/g,'&quot;')}" alt="${name.replace(/&/g,'&amp;').replace(/"/g,'&quot;')}" style="max-height:42px;max-width:150px;object-fit:contain;vertical-align:middle">`
